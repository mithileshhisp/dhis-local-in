    ...
    */
        ...
        throws Exception
    {
        ...
        */
        ...
        else if ( aggType.equals( Lookup.CCEI_AGG_TYPE_REF_WORKING_STATUS ) )
        {
            ...
            String query = cceiAggregationService.getQueryForRefrigeratorWorkingStatus( equipmentTypeId, workingStatus );
        ...
        else if ( aggType.equals( Lookup.CCEI_AGG_TYPE_REF_FAULT_TYPES ) )
        {
            HttpServletRequest request = ServletActionContext.getRequest();
            
            Lookup lookup = lookupService.getLookupByName( Lookup.CCEI_FAULT_TYPE_OPTIONSET );
            
            String[] faultTypeValues = request.getParameterValues( lookup.getName() );
            String faultTypes = "";
            for( String faultTypeValue : faultTypeValues )
            {
                faultTypes += "'" + faultTypeValue + "',";
            }
            ...
            String query = cceiAggregationService.getQueryForRefrigeratorFaultTypes( equipmentTypeId, faultTypes );
            
            CaseAggregationCondition condition = new CaseAggregationCondition( dataElement.getName(), aggType, query, dataElement, dataElementCategoryService.getDefaultDataElementCategoryOptionCombo() );
            
            aggregationConditionService.addCaseAggregationCondition( condition );
        }        
        else if ( aggType.equals( Lookup.CCEI_AGG_TYPE_REF_PARTS_REPLACED ) )
        {
            HttpServletRequest request = ServletActionContext.getRequest();
            
            Lookup lookup = lookupService.getLookupByName( Lookup.CCEI_PARTS_REPLACED_OPTIONSET );
            
            String[] partsReplacedValues = request.getParameterValues( lookup.getName() );
            String partsReplaced = "";
            for( String partsReplacedValue : partsReplacedValues )
            {
                partsReplaced += "'" + partsReplacedValue + ";',";
            }
            
            partsReplaced = partsReplaced.substring( 0, partsReplaced.length()-1 );
            
            lookup = lookupService.getLookupByName( Lookup.CCEI_REF_EQUIPMENTTYPE_ID );
            
            Integer equipmentTypeId = Integer.parseInt( lookup.getValue() );
            
            String query = cceiAggregationService.getQueryForRefrigeratorPartsReplaced( equipmentTypeId, partsReplaced );
