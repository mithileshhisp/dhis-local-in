    ...
    implements Action
{
        ...
        throws Exception
    {
		...
		if( period == null )
		{
		    ...
		    period = PeriodType.getPeriodFromIsoString( curMonth );
		    period = periodService.reloadPeriod( period );
		}
        ...
        if( orgUnitList != null && orgUnitList.size() > 0 )
        {
            for ( CaseAggregationCondition condition : conditions )
            {
                ...
                else if( condition.getOperator().equals( Lookup.CCEI_AGG_TYPE_REF_WORKING_STATUS ) || condition.getOperator().equals( Lookup.CCEI_AGG_TYPE_REF_FAULT_TYPES ) || condition.getOperator().equals( Lookup.CCEI_AGG_TYPE_REF_PARTS_REPLACED ) )
                {
