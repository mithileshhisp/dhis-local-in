    ...
    implements CCEIAggregationService
{
    ...
    public Map<String, Integer> calculateStorageCapacityData( Period period, DataElement dataElement, Set<OrganisationUnit> orgUnits, Set<OrganisationUnitGroup> orgUnitGroups )
    {
        ...
        */
        ...
        Map<String, String> dataValueMapForRequirement = new HashMap<String, String>( ccemReportManager.getDataValueMapForRequirement( orgUnitIdsByComma, deIds, periodId ) );
        ...
        for ( OrganisationUnit orgUnit : orgUnits )
        {
            ...
            Lookup lookupActual = lookupService.getLookupByName( Lookup.CCEI_AGG_TYPE_STORAGE_CAPACITY_ACTUAL );
            Lookup lookupRequirement = lookupService.getLookupByName( Lookup.CCEI_AGG_TYPE_STORAGE_CAPACITY_REQUIREMENT );
            
            if( lookupActual != null && lookupActual.getValue() != null )
            {
                if( vaccineActualValue.intValue() > 0 )
                {
                    aggregationResultMap.put( orgUnit.getId() + ":" + lookupActual.getValue(),  vaccineActualValue.intValue()  );
                }
                
            }
            
            if( lookupRequirement != null && lookupRequirement.getValue() != null )
            {
                if( vaccineRequirement.intValue() > 0 )
                {
                    aggregationResultMap.put( orgUnit.getId() + ":" + lookupRequirement.getValue(), vaccineRequirement.intValue() );
                }
    ...
    // calculate Refrigerator Count ByStatus And Facilty
    public void calculateRefrigeratorCountByStatusAndFacilty( Period period, Set<OrganisationUnit> orgUnits)
    {
        Map<String, Integer> aggregationResultMap = new HashMap<String, Integer>();
        ...
        OptionSet optionSet = optionService.getOptionSet( Integer.parseInt( lookup.getValue() ) );
        
        lookup = lookupService.getLookupByName( Lookup.CCEI_AGG_REF_DATAELEMENT_IDS );
        
        String dataElementIds = lookup.getValue();
        
        String[] deIds = dataElementIds.split( ":" );
        
        for( String optionName : optionSet.getOptions() )
        {
            if( optionName.equalsIgnoreCase( Lookup.CCEI_REF_EQUIPMENT_FUCTIONING ) )
            {
                try
                {
                    query = " SELECT equipment.organisationunitid,COUNT(*) FROM equipment " +
                            " INNER JOIN equipmentattributevalue ON equipmentattributevalue.equipmentid = equipment.equipmentid " +
                            " WHERE equipment.equipmenttypeid = " + equipmentTypeId + " AND " +
                            " equipment.organisationunitid IN ( " + orgUnitIdsByComma + " ) AND " +
                            " equipmentattributevalue.value IN ( '" + Lookup.CCEI_REF_EQUIPMENT_FUCTIONING + "' ) AND " +
                            " equipment.registrationdate <= '" + simpleDateFormat.format( period.getEndDate() ) +"'" +
                            " GROUP BY equipment.organisationunitid";
                    ...
                    while ( rs.next() )
                    {
                        ...
                        if( count != null  )
                        {
                            aggregationResultMap.put( orgUnitId+ ":" + deIds[0], count );
                        }
                    }
            ...
            else if( optionName.equalsIgnoreCase( Lookup.CCEI_REF_EQUIPMENT_AWAITING_REPAIR ) )
            {
                try
                {
                    query =  " SELECT equipment.organisationunitid,COUNT(*) FROM equipment " +
                             ...
                             " WHERE equipment.equipmenttypeid = " + equipmentTypeId + " AND " +
                             " equipment.organisationunitid IN ( " + orgUnitIdsByComma + " ) AND " +
                             " equipmentattributevalue.value IN ( '" + Lookup.CCEI_REF_EQUIPMENT_AWAITING_REPAIR + "' ) AND " +
                             " equipment.registrationdate <= '" + simpleDateFormat.format( period.getEndDate() ) +"'" +
                             " GROUP BY equipment.organisationunitid";
                    ...
                    while ( rs.next() )
                    {
                        ...
                        if( count != null  )
                        {
                            aggregationResultMap.put( orgUnitId+ ":" + deIds[1], count );
                        }
                    }
            ...
            else if( optionName.equalsIgnoreCase( Lookup.CCEI_REF_EQUIPMENT_UNSERVICABLE ) )
            {    
                try
                {
                    query =  " SELECT equipment.organisationunitid,COUNT(*) FROM equipment " +
                             ...
                             " WHERE equipment.equipmenttypeid = " + equipmentTypeId + " AND " +
                             " equipment.organisationunitid IN ( " + orgUnitIdsByComma + " ) AND " +
                             " equipmentattributevalue.value IN ( '" + Lookup.CCEI_REF_EQUIPMENT_UNSERVICABLE + "' ) AND " +
                             " equipment.registrationdate <= '" + simpleDateFormat.format( period.getEndDate() ) +"'" +
                             " GROUP BY equipment.organisationunitid";    
                    ...
                    while ( rs.next() )
                    {
                        ...
                        if( count != null  )
                        {
                            aggregationResultMap.put( orgUnitId+ ":" + deIds[2], count );
                        }
                    }
        ...
        if( Lookup.CCEI_REF_EQUIPMENT_TOTAL.equalsIgnoreCase( "TOTAL" ) ) 
        {    
            try
            {
                query =  " SELECT equipment.organisationunitid,COUNT(*) FROM equipment " +
                ...
                while ( rs.next() )
                {
                    ...
                    if( count != null  )
                    {
                        aggregationResultMap.put( orgUnitId+ ":" + deIds[3], count );
                    }
                }
        ...
        String importAggStatus = importData( aggregationResultMap, period );
        
        System.out.println( " Import Refrigerator Count By Status And Facilty " + importAggStatus );
        
    }
        
    // importing data in data value table
