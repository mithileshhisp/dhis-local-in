...
{   
	...
	//document.getElementById('selOrgunitID').value = orgUnits;
...
function showEquipmentStatusForm( equipmentId, type )
{
	var d_height = 350;
	var d_width = 600;
	...
	//document.getElementById('overlay').style.visibility = 'visible';
	jQuery('<div id="editEquipmentStatusDiv">' ).load( 'showEquipmentStatusForm.action?equipmentId='+ equipmentId + "&type=" + type ).dialog({
		title: tempModelName + " at " + tempClinicName,
		maximize: true,
		closable: true,
		modal:true,
...
function updateEquipmentStatus()
{
	...
	$.ajax({
      ...
      data: getParamsForEquipmentStatusDiv('editEquipmentStatusDiv'),
...
//
function getParamsForEquipmentStatusDiv( equipmentDiv )
{
	var params = '';
	...
	jQuery("#" + equipmentDiv + " :input").each(function()
		{
			...
			else if( $(this).attr('type') == 'hidden' || $(this).attr('type') == 'text' )
			{
				var value = "";
				if( jQuery(this).val() != '' )
				{
	...
	//alert( "1. " + params );
	elementId = $("#description").attr('id');
	params += elementId + "=" + $("#description").val();

	//alert( "2. " + params );	
	params += "&" + getParamsStringBySelected( 'reasonNotWorking', 'reasonNotWorking' );
	
	//alert( "3. " + params );	
	params += "&" + getParamsStringBySelected( 'utilization', 'utilization' );
		
	//alert( "4. " + params );
	params += "&" + getParamsStringBySelected( 'workingStatus', 'workingStatus' );			
		
	//alert( "5. " + params );	
	
	if( $("#type").val() == "repair" )
	{ 	
		params += "&" + getParamsStringBySelected( 'faultTypes', 'faultTypes' );
		
		params += "&" + getParamsStringBySelected( 'partsReplaced', 'partsReplaced' );
							
	}
	else if( $("#type").val() == "service" )
	{
		params += "&" + getParamsStringBySelected( 'serviceTypes', 'serviceTypes' );				
	}	
...
function equipmentStatusDateValidation()
{
	var requestedDate = document.getElementById( "dateOfService" ).value;
    
	var reportingDate = document.getElementById( "serviceReportingDate" ).value;
	
	//alert( requestedDate + "--" + reportingDate );
	
	var  repairRequestedDate = new Date( requestedDate );
	var  serviceReportingDate = new Date( reportingDate );
	
	//alert( repairRequestedDate + "--" + serviceReportingDate );
	
	//var now = new Date();
	
	var diff = repairRequestedDate.getTime() - serviceReportingDate.getTime();
	
	//alert( diff );
	
	if ( repairRequestedDate.getTime() > serviceReportingDate.getTime() ) 
	{ 	
		showWarningMessage( "Date Repaired not less than Requested Date " );
		document.getElementById( "serviceReportingDate" ).value = "";
...
function showAddEquipmentForm( orgUnitId, orgUnitName )
{
	//var orgUnitId = document.getElementById('selectedOrgunitID').value;
...
function showAddEquipmentIcePacksForm( orgUnitId, orgUnitName )
{
	//var orgUnitId = document.getElementById('selectedOrgunitID').value;
...
//Equipment Move Form
function showEquipmentMoveForm( equipmentId , clinicName, modelName )
{
	var orgUnitName = clinicName;
	var equipmentModelName = modelName;
	...
	jQuery('#editEquipmentDiv').dialog('destroy').remove();
	jQuery('<div id="editEquipmentDiv">' ).load( 'showEquipmentMoveForm.action?equipmentId='+ equipmentId ).dialog({
		title: equipmentModelName + " at " + orgUnitName + " for move",
		maximize: true,
		closable: true,
		modal:true,
		overlay:{background:'#000000', opacity:0.1},
...
function validateEquipmentMove()
{
	var elementId = '';
	elementId = $("#equipmentID").attr('id');
	
	var orgUnitId = selectionTreeSelection.getSelectedUid()[0];
	
	var url = "validateEquipmentMove.action?";
		url += elementId + "=" + $("#equipmentID").val();
...
// Equipment Move
function equipmentMove()
{
	
	$.ajax({
	    type: "POST",
	    url: 'equipmentMove.action',
	    data: getParamsForEquipmentMoveDiv('editEquipmentDiv'),
	    	success: function( json ) {
			//loadAllEquipments();
...
// get parameter for Equipment move
function getParamsForEquipmentMoveDiv( equipmentDiv )
{
	var params = '';
	var elementId = '';

	elementId = $("#equipmentID").attr('id');
	params += elementId + "=" + $("#equipmentID").val();
	
	
	//params += "&" + "orgUnitId="+ selectionTreeSelection.getSelectedUid()[0];
