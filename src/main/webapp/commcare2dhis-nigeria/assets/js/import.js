   ...
   // console.log('Pre-OU ID List: '+orgUnitIDList);
    ...
    for(var item in formData.objects)
    {
        ...
        if(formData.objects[item].form['@name']==dataFormName)
        {
            ...
            ++totalFormCount;
            var facility_name = formData.objects[item].form.case['@case_id'];
    ...
    for(var item in formData.objects)
    {
        ...
        if(formData.objects[item].form['@name']==dataFormName )
        {
           ...
           //{
               ...
               //-------------- /Gp_SOR_Admit/ ----------------//
               commCareValueMap.set('XNV6X8j6H7X',formData.objects[item].form.Gp_SOR_Admit.A1_open);
               ...
               commCareValueMap.set('WoJlrC1wWlM',formData.objects[item].form.Gp_SOR_Staff.S12_blood_staff);
...
var fetchDHISOrgUnitList = function () {
    ...
    // url for fetch orgUnit at level 1 http://hospdev.hispindia.org/quic/api/organisationUnits.json?filter=level:eq:1&fields=id,name
        //http://hospdev.hispindia.org/quic/api/organisationUnits.jsonp?filter=level:eq:1&fields=id,name&paging=false
...
// get DHIS Root orgUnitUid

var getDHISRootOrgUnitUid = function () {

    // url for fetch orgUnit at level 1 http://hospdev.hispindia.org/quic/api/organisationUnits.json?filter=level:eq:1&fields=id,name
    //http://hospdev.hispindia.org/quic/api/organisationUnits.jsonp?filter=level:eq:1&fields=id,name&paging=false
    var rootOrgUnitURL = baseURL+"/organisationUnits.jsonp?filter=level:eq:1&fields=id,name&paging=false";
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for(var orgUnit in orgUnitList) {
                if (orgUnitList[orgUnit].id != null)
                {
                    rootOrgUnitUid = orgUnitList[orgUnit].id;
                    //rootOrgUnitUid = orgUnitList[orgUnit];
                    console.log( "Root OrgUnit Uid -- " + rootOrgUnitUid);
                    return;
                }
            }
        }
...
$(document).ready(function () {
    ...
    var commCareFormURL = "https://www.commcarehq.org/a/nigeria-quic/api/v0.4/form/?limit=1000"; //?received_on_start="+startDate+"received_on_end="+endDate;
    var commCareUserID = "ghongane.ganesh@gmail.com";
    var commCarePassword = "ganesh123";
