...
.controller('SelectionController',
    ...
    $rootScope.CCEI_CONFIGURATION_KEY = "ccei-configuraton-json";
    $rootScope.ConfigurationSettings = {};
    $rootScope.ConfigurationSettings.parameters=[];

    CCEIConfigurationSetting.get($rootScope.CCEI_CONFIGURATION_KEY).then(function(conf){
        for (var count in conf.parameters){
            $rootScope.ConfigurationSettings.parameters[conf.parameters[count].key] = conf.parameters[count];

            if ( conf.parameters[count].key == 'model-uid' ){
                $scope.modelTrackedEntityId = conf.parameters[count].value;
            }
        }
    });
    ...
    $scope.$watch('selectedOrgUnit', function() {           
        ...
        if( angular.isObject($scope.selectedOrgUnit)){   
            ...
            // for CCEI
            $rootScope.showModalFlag =  SessionStorageService.get('SHOW_MODEL_FLAG');
    ...
    // CCEI Related Functions
    if ($scope.trackerAssociateInstancesMap.length == 0)
        CCEIOrgUnitFactory.getRootOu().then(function (ou) {
            AttributesFactory.getAll().then(function (atts) {
                angular.forEach(atts, function (att) {
                    if (att.valueType == 'TRACKER_ASSOCIATE') {
                        CCEIConfigurationSetting.getMappedProgram($scope.selectedProgram.id).then(function(modelProgramId) {
                            CCEIProgram.getTrackedEntityInstancesByProgram(modelProgramId, ou.id).then(function (teis) {
                                var trackerAssociateAttributeIndex = 5;//attributes start from 5th index through the API
                                for (var count in teis.headers) {
                                    if (teis.headers[count].name == $rootScope.ConfigurationSettings.parameters['tracker-associate-attribute-display'].value) {
                                        trackerAssociateAttributeIndex = count;
                                    }
                                }
                                for (var count in teis.rows) {

                                    var tei = teis.rows[count];
                                    $scope.trackerAssociateInstancesMap[tei[0]] = tei[trackerAssociateAttributeIndex];
                                }
    ...
    $scope.loadPrograms = function(orgUnit) {
        ...
        if (angular.isObject($scope.selectedOrgUnit)) {   
            ...
            // for CCEI
            CCEIProgram.getProgramsByOuAndTe($scope.selectedOrgUnit, $scope.selectedProgram,$scope.modelTrackedEntityId,$rootScope.showModalFlag).then(function(response){
    ...
    $scope.search = function(mode){        
		...
		// for CCEI
        if ($rootScope.showModalFlag){
            $scope.teUrl = "trackedEntity=" + $scope.modelTrackedEntityId;
        }
		// end
    ...
    $scope.fetchTeis = function(){
        ...
        TEIService.search($scope.searchingOrgUnit.id, 
                ...
                //behaviour modification for ccei
                if(data.rows && !$rootScope.showModalFlag){
                    for (var i=0;i<data.rows.length;i++){
                        if (data.rows[i][4] == $scope.modelTrackedEntityId){
                            data.rows.splice(i,1); i--;
                        }
                    }
                    $scope.teiCount = data.rows.length;
                    data.metaData.pager.total = data.rows.length;
                }
