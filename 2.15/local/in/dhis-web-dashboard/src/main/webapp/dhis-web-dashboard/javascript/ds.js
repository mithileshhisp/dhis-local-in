...
{	
    ...
    var dataSetPeriodType = xmlObject.getElementsByTagName( "dataSetPeriodType" );
    
    var dsPeriodType = "";
    
    for ( var i = 0; i < dataSetPeriodType.length; i++ )
    {
    	dsPeriodType = dataSetPeriodType[ i ].getElementsByTagName("periodTypeName")[0].firstChild.nodeValue;
    }    	
    
    //alert( dsPeriodType );
    
    if( dsPeriodType == "Weekly" )
    {
    	showById('yearMonth');
...
function getWeeklyPeriods()
{
	...
	var startPeriods = document.getElementById( "sDateLB" );
	var endPeriods = document.getElementById( "eDateLB" );
	...
	{
	   clearList( startPeriods );
	   clearList( endPeriods );
	}
...
{	
	...
	for ( var i = 0; i < periods.length; i++)
	{
		...
		var startDateOption = document.createElement( "option" );
		startDateOption.value = id;
		startDateOption.text = periodName;
        sDateLB.add( startDateOption, null );
			
        var endDateOption = document.createElement( "option" );
        endDateOption.value = id;
        endDateOption.text = periodName;
        eDateLB.add( endDateOption, null);
